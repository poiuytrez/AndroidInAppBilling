...
public class IabHelper {
	...
	public int RestoreTransaction(Inventory inv, String itemType)
			throws JSONException, RemoteException {
		// Query purchases
		logDebug("Querying owned items, item type: " + itemType);
		logDebug("Package name: " + mContext.getPackageName());
		...
		do {
			...
			for (int i = 0; i < purchaseDataList.size(); ++i) {
				...
				if (Security.verifyPurchase(mSignatureBase64, purchaseData,
					...
					try {
						int response1 = mService.consumePurchase(3,
								mContext.getPackageName(), purchase.getToken());
						logDebug("inside run response" + response);
					} catch (RemoteException e) {
						// TODO Auto-generated catch block
						logDebug("exception here 1");
						e.printStackTrace();
					}
	...
	public int querySkuDetails(String itemType, Inventory inv,
			List<String> moreSkus) throws RemoteException, JSONException {
	...
	public ArrayList<String> querySkuDetails1(String itemType, Inventory inv,
			List<String> moreSkus) throws RemoteException, JSONException {
		logDebug("Querying SKU details.");
		ArrayList<String> skuList = new ArrayList<String>();
